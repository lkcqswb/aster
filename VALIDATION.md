# Keyboard, layout, limits, auto-compact, tools, models and 弄玉's presence 0.18 — 2026-09-24

- **155 Rust tests passed** (1 ignored: the Chromium renderer check, run separately below). Formatting, strict Clippy and the release build passed. New coverage: composer editing (line motion, prompt history with draft restore, word motion including Option-arrows, kill line/word, wide-character wrapping), bounded transcript scrolling, session picker selection/deletion, every slash command and F1 action giving visible feedback, turn limits, auto-compact (demo summary, disabled per session/globally, no loop when nothing can be archived, mid-turn continuation, local fallback after a failed summary, and a model summary through a local mock provider: one request, no tools, nothing streamed), the six new tools, the models registry (masked keys, https/localhost-only bases, owner-only file, edits keep keys) and panel, and emotion cues (hidden while streaming, other brackets untouched).
- **Key routing, end to end in the app:** a provider added through `/models` with `x-api-key` auth received the request with exactly that key in `x-api-key`, no `Authorization` header, `anthropic-version: 2023-06-01` and the chosen model. The saved session and its export contained neither the key nor the reply's emotion cue. The local provider was a test server on 127.0.0.1; no external API was called.
- **Emotions end to end:** `scripts/test_emotion_e2e.py` streams a reply containing `〔happy〕` and `[[wave]]` from a local stand-in provider. The transcript showed only the words, her card showed "happy", and the renderer (stub rig in headless Chromium) reported the `happy` emotion and played `nod` (on sending), `wave` (from the cue) and then `/act bow`. The key arrived only in `x-api-key`; the cue stayed in the saved provider blocks. The helper's stub-rig renderer check also passed here (91 s).
- **A lost Enter in the Files panel:** the final regression caught `test_decision_review_e2e` failing intermittently (1 of 6 runs) where Enter on a filtered file did nothing. Cause: Enter was ignored while the filter's results were refreshing, even though the previous results were still shown. Enter now opens the selection once the fresh results arrive (typing again cancels it). A unit test covers it; afterwards the suite passed 8 of 8 runs.
- **Real PTY suites, all passing (18 + the `--live2d` variant):** emotion cues, keyboard/sessions, auto-compact, main flow, commands, context, queue, tasks, checkpoint, decision review, evidence, history, navigator, rich text, terminal, companion work, actions and shutdown. Suites that need animation frames ran against a **stub Live2D rig** (a test double for Pixi/Cubism in `tests/fixtures/live2d-stub.js`) through the real asset server, headless Chromium, CDP and frame pipeline. They are not a test of the user's real model.
- **Renderer check in headless Chromium with the stub rig** (`ASTER_TEST_CHROME=… cargo test stub_renderer -- --ignored`), run in this environment: 14.8 fps reported (14.1 at 420×620 PNG, 13.3 at 640×940), about 29 ms per frame, 270 frames sampled, largest head step across an idle→thinking switch 0.44° per frame, 2 blinks in 7 s, mouth closed while not speaking and peaking at 0.88 with 22 reversals in 3 s of streamed text.
- **Whole-screen blinks:** measured by counting full-screen clears (`ESC[2J`) in the PTY output with the renderer streaming frames. Before: 4 while typing a three-line draft, 1 when clearing it, 1 opening the session picker. After: **0 in every phase** (startup, idle, typing, clearing, /help, sessions, approval, command output, palette), with frames continuing at about 15 per second while panels are open.
- **Auto-compact in the terminal:** a 40,000-token window at 50% compacted automatically before the second request (89 → 45 KB), archived the full context with owner-only permissions and showed the checkpoint report; demo mode, no API call.
- **Not verified here:** the real 弄玉 rig's appearance, framing, expressions and gestures (her model files are only on the user's machine); flicker in real iTerm2/Kitty/Ghostty; a live MiniMax or Anthropic request with a real key (none was made); the model-written summary's quality on a real conversation.
