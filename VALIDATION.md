...
This merges 0.19's Companion API into the 0.18 follow-up work. 弄玉's emotions and gestures now come only from explicit controls through that API (`/emotion`, `/motion`, `Companion::control`); Aster no longer chooses them itself. The inline `〔cue〕` convention and its prompt text, tone guessing from replies, event reactions, idle gestures, the "sleepy" timer, the keyword-discovered 18 emotions and 12 gestures, `/act`, `/pet rig` and the `aster-nongyu.json` override file were removed. `companion.js`, the profiles, schema, examples and `src/companion.rs` are unchanged from 0.19.

- **151 Rust tests passed** (1 ignored: the Chromium renderer check, run separately below). Formatting, strict Clippy and the release build passed. **7 JavaScript contract tests** for `companion.js` passed. `--check-companion` accepts the default profile (4 emotions, 3 motions, 16 bindings).
...
- **Real PTY suites, all passing (17 + the `--live2d` variant):** keyboard/sessions, auto-compact, main flow, commands, context, queue, tasks, checkpoint, decision review, evidence, history, navigator, rich text, terminal, companion work, actions and shutdown. Suites that need animation frames ran against the stub rig (`tests/fixtures/live2d-stub.js`, a test double for Pixi/Cubism with the default profile's parameter ids) through the real asset server, headless Chromium, CDP and frame pipeline.
