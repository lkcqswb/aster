# Keyboard, layout, tools, models and the renderer on the Companion API 0.20 — 2026-09-24

This merges 0.19's Companion API into the 0.18 follow-up work. 弄玉's emotions and gestures now come from the conversation's provider (one structured request after each reply) or from explicit controls (`/emotion`, `/motion`), always through that API; Aster no longer chooses them itself. The inline `〔cue〕` convention and its prompt text, tone guessing from replies, event reactions, idle gestures, the "sleepy" timer, the keyword-discovered 18 emotions and 12 gestures, `/act`, `/pet rig` and the `aster-nongyu.json` override file were removed. `companion.js`, the profiles, schema, examples and `src/companion.rs` are unchanged from 0.19.

- **The upstream expression query** (`src/expression.rs`): after a reply, one structured request (`output_config.format`, a JSON Schema whose `emotion`/`motion` enums come from the profile; strength checked locally because structured outputs do not enforce numeric bounds) to the conversation's own provider, with no tools, no streaming, `max_tokens` 1024, a 30 s timeout and no retry. Unit tests cover the request shape, bounded input, exact validation (unknown names, out-of-range or non-numeric strength, refusal, truncation, prose-wrapped JSON from providers without structured outputs), a key that never appears in errors, and which failures pause the query. A UI test runs a real turn against a local provider and checks the two controls queued for the renderer, the untouched UI mood, the counted tokens, and that nothing reached the transcript or session file. An answer that arrives after the conversation changed is counted toward the conversation it was asked for and not shown in the new one.
- **Expression query end to end** (`scripts/test_expression_e2e.py`): the release binary in a real PTY, a local stand-in provider and the stub rig in headless Chromium. The provider saw exactly two requests (the streamed turn, then the structured query with the profile's enums and no tools); the renderer confirmed `happy` at 0.7 and acknowledged `nod`; her card showed the confirmed emotion and `/status` the query's result; the transcript held only the reply; usage counted both requests (370 input / 36 output tokens); 172 frames; exit 0; no external API call.
- **158 Rust tests passed** (1 ignored: the Chromium renderer check, run separately below). Formatting, strict Clippy and the release build passed. **7 JavaScript contract tests** for `companion.js` passed. `--check-companion` accepts the default profile (4 emotions, 3 motions, 16 bindings).
- **A lost emotion strength, found in review and fixed:** `/emotion focused 0.6` queues the control and also sets the UI's mood, which reaches the renderer on the next UI tick. When a frame was drawn in between, that later mood re-applied the emotion at strength 1 and replaced the control's receipt. The renderer now applies the mood only when she is not already showing it, after the frame's controls. The renderer check below sends the control, waits for its receipt, then sends the mood: with 0.19's line restored it **failed** ("mood kept strength"); with the fix it passes and the smile stays at 0.225 for strength 0.5.
- **Renderer check in headless Chromium with the stub rig** (`ASTER_TEST_CHROME=… cargo test stub_renderer -- --ignored`): every default binding present (no warnings); idle sway 0.61° with at most 0.04° per frame; `happy` at 0.5 wrote 0.225 to both `ParamEyeSmile` and `ParamEyeSmileR`; `nod` acknowledged with 1100 ms and peaked at 3.4°, then ended; an unknown emotion was refused with its reason; reset returned the smile to 0; the canvas followed a 300×500 pane at the profile's zoom; unbound parameters never moved; 15 fps (about 7 ms per frame); JPEG at 640×940 also 15 fps.
- **The 0.19 companion terminal test** (`scripts/test_companion_api_e2e.py`) passed against the stub rig standing in for `~/desktop-pet`: custom emotion strength acknowledged, all channels supported, motion and reset acknowledged, unknown name and out-of-range strength rejected, 43 distinct frames, resize, exit 0, no API calls. Its asset-hash check covered the stub's 4 files, not the real model.
- **Probe:** `--live2d-probe --probe-emotion happy --probe-motion nod` on the stub acknowledged reset, `happy` (0.7 → 0.315 on `ParamEyeSmile`, the value 0.19 measured on the real rig), `nod`, gaze and reset, and wrote all preview frames.
- **Real PTY suites, all passing (19 + the `--live2d` variant):** keyboard/sessions, auto-compact, main flow, commands, context, queue, tasks, checkpoint, decision review, evidence, history, navigator, rich text, terminal, companion work, actions, shutdown, the expression query and the 0.19 companion test. Suites that need animation frames ran against the stub rig (`tests/fixtures/live2d-stub.js`, a test double for Pixi/Cubism with the default profile's parameter ids) through the real asset server, headless Chromium, CDP and frame pipeline.
- **Whole-screen blinks:** full-screen clears (`ESC[2J`) counted in the PTY output while frames stream: **0 in every phase** (startup, idle, typing, clearing, /help, sessions, approval, command output, palette), with about 15 frames per second throughout.
- **Key routing:** a provider added through `/models` with `x-api-key` auth received the request with exactly that key in `x-api-key`, no `Authorization` header, `anthropic-version: 2023-06-01` and the chosen model; the saved session and its export did not contain the key. The provider was a local test server; no external API was called.
- **Not verified here:** the real 弄玉 rig's appearance and motion (her model files are only on the user's machine); flicker in real iTerm2/Kitty/Ghostty; a live MiniMax or Anthropic request with a real key; the model-written summary's quality on a real conversation. Whether a real MiniMax or Anthropic endpoint accepts `output_config.format` for the chosen model (a rejection pauses the query with a notice; no live request was made).

# Public companion interfaces 0.19 — 2026-09-24

- **108 Rust tests passed**, covering the profile/command contract and custom model-file routes, alongside the existing harness suite. **7 asset-free JavaScript contract tests** verify interpolation, strengths, expiry, reset, per-rig clamps, unsupported channels and invalid inputs. Formatting, strict Clippy and release build passed.
- Two actual-model probes exported **314 parameters** and acknowledged custom `focused`/`acknowledge`, default `happy`/`nod`, timed gaze and reset. Introspection found the previous renderer's `ParamEyeLSmile`/`ParamEyeRSmile` did not exist. The default bindings now use the actual `ParamEyeSmile`/`ParamEyeSmileR`; both final profiles report **zero unsupported bindings**. At strength 0.7, focused wrote 0.105 and happy wrote 0.315 to both eye-smile parameters. The emitted portrait frames were visually inspected. Probe directories: `.aster/qa/companion-api` and `.aster/qa/companion-happy`.
- The custom-profile animated terminal test passed with **44 distinct frames**, exact emotion strength acknowledgement, all expected supported channels, named motion/reset responses, rejected unknown names and out-of-range strength, resize and exit 0. **13 source asset hashes were unchanged**, and no provider requests or tokens were used. Evidence: `.aster/qa/companion-api-e2e.json`.
- The first terminal run hit the previously observed texture-loading timeout (7/9 textures), including its bounded startup retry. Both 2048-pixel probes succeeded; the isolated terminal control regression passed with a 1024-pixel test profile. This does not establish or fix the intermittent renderer startup cause. The general terminal regression also passed.
- Public artifacts contain only interface source, profiles, schemas, examples and documentation. Probe images, exported rig metadata, model files and vendor SDKs remain local. Preview acknowledgements verify parameter application, not subjective animation quality or project correctness.
