...
class RemoteBackend:
    ...
    def __init__(self, url, expected_workspace=None):
        ...
        if (parsed.scheme != "http" or parsed.hostname not in {"127.0.0.1", "localhost"}
            ...
            raise ValueError("Connect to a local Relay address such as http://127.0.0.1:8787.")
        self.url = url.rstrip("/")
        self.opener = urllib.request.build_opener(urllib.request.ProxyHandler({}), NoRedirect)
        config = self.config()
        if config.get("app") != "relay" or config.get("protocol") != 1:
            raise ValueError("This is not a compatible Relay server. Restart the dashboard with the updated code.")
    ...
    def request(self, path, body=None):
        ...
        try:
            ...
            try:
                message = json.loads(exc.read(4096)).get("error", f"Relay returned HTTP {exc.code}.")
            ...
            raise ConnectionError("Cannot reach the Relay dashboard. Check that it is still running.") from None
...
class NoRedirect(urllib.request.HTTPRedirectHandler):
    def redirect_request(self, *args, **kwargs):
        raise ValueError("Relay does not follow server redirects.")
