    ...
    with tempfile.TemporaryDirectory(prefix='aster-edit-live-') as tmp:
        ...
        try:
            ...
            while time.monotonic()<deadline:
                ...
                if 'Allow edit_file?' in text and not approved_edit:
                ...
                if 'Allow shell?' in text and not approved_shell:
            ...
            evidence={'session':result['id'],'model':result['model'],'model_turns':sum(m['role']=='assistant' for m in result['messages']),'tools':result['tools'],'input_tokens':result['input_tokens'],'output_tokens':result['output_tokens'],'plan':result['work']['steps'],'approved_edit':approved_edit,'approved_test_command':approved_shell,'tests_unchanged':True,'independent_test_exit':validation.returncode,'test_output':validation.stderr,'source':(project/'calculate.py').read_text(),'exit':child.exitstatus}
            (qa/'agent-edit-live.json').write_text(json.dumps(evidence,ensure_ascii=False,indent=2))
